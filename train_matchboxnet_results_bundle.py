...
RESULTS_ROOT = os.path.join("data", "results", "matchboxnet_runs")
...
class RunPaths:
    def __init__(self, results_root: str, run_name: str):
        ...
        self.checkpoint = os.path.join(self.run_dir, "matchboxnet_best.pt")
...
class TinyMatchboxNet(nn.Module):
...
def main():
    ...
    model = TinyMatchboxNet(num_classes=len(LABELS)).to(device)
    ...
    with open(paths.summary_txt, "w", encoding="utf-8") as f:
        f.write("MatchboxNet training summary\n")
        f.write("=" * 30 + "\n\n")
